...
class RouteSubscriber implements SubscriberInterface
{
    ...
    public function onPostDispatch(\Enlight_Controller_ActionEventArgs $args)
    {
        ...
        if ($controller->Request()->getActionName() === 'index' && $controller->Request()->getControllerName() === 'index') {
            if ($view->templateExists('frontend/myPlugin/popup.tpl')) {
                $view->extendsTemplate('frontend/myPlugin/popup.tpl');
            } else {
                $view->addTemplateDir($this->pluginDirectory . '/Resources/views/frontend/index/');
            }
        }
    }
}
